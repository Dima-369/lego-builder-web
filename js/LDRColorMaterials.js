...
LDR.Colors.getColor4 = function (colorID) {
  ...
  let color = new THREE.Color(
    EDGE ? (colorObject.edge ? colorObject.edge : 0x333333) : colorObject.value,
  );
...
LDR.Colors.buildLineMaterial = function (c, conditional) {
  let isDarkBlue = c === 272 || c === -273; // 272 is Dark Blue, -273 is its edge ID
  let color =
    (LDR.Options && LDR.Options.lineContrast === 0) || isDarkBlue
...
LDR.Colors.buildTriangleMaterial = function (c, texmap) {
  ...
  if (texmap) {
    uniforms["map"] = {
      type: "t",
      value: texmap === true ? LDR.Colors.tempTexure : texmap,
    };
  }
  ...
  let ret = new THREE.RawShaderMaterial({
    ...
    fragmentShader: texmap
      ? LDR.Shader.TextureFragmentShader
      : LDR.Shader.SimpleFragmentShader,
    transparent: isTrans,
    depthWrite: !isTrans,
    polygonOffset: true,
...
LDR.Colors.logoPositions = [
  ...
  [-2, 0, -2, -2, 2, -3, 2, -1],
  ,
  [-1.5, 2.25, -2, 1.5, -1.5, 0.5, 1.5, -0.25, 2, 0.5, 1.5, 1.5, 0, 2, 0, 1], //G
  [-1.5, 4.75, -2, 4, -1.5, 3, 1.5, 2.25, 2, 3, 1.5, 4, -1.5, 4.75], // O
];
...
LDR.Colors.createRandomTexture = function (damage, waves, waveSize, speckle) {
  ...
  for (let y = 1; y < size; y++) {
    for (let x = 1; x < size; x++) {
      ...
      let v = new THREE.Vector3(
        -(a[2] + a[8] - a[6] - a[0] + 2 * (a[5] - a[3])),
        a[6] + a[8] - a[2] - a[0] + 2 * (a[7] - a[1]),
        1,
      ); // Sample left/right neighbours and weight direct neighbours the most.
  ...
  if (speckle) {
    ...
    let speckleSize =
      (speckle.minSize + speckle.maxSize) * SPECKLE_MULT * SPECKLE_MULT; // Magic number used since this can be stretched onto parts of arbitrary sizes.
    let numSpeckles = Math.floor(
      (size * size * speckle.fraction) / speckleSize,
    );
    console.log(
      "Applying " + numSpeckles + " speckles of average size " + speckleSize,
    );
    for (let i = 0; i < numSpeckles; i++) {
      ...
      let diam =
        SPECKLE_MULT *
        (speckle.minSize + Math.random() * (speckle.maxSize - speckle.minSize));
...
LDR.Colors.listeningMaterials = {
  trans: [], // Only one trans texture.
  opaque: [], // -||-
  pearl: [],
  rubber: [],
  metal: [],
  speckle: {},
}; // Speckle materials ordered by colorID: colorID->[]
...
LDR.Colors.loadEnvMapTextures = function (render) {
  ...
  if (LDR.Colors.reflectionCube) {
    ...
    new THREE.CubeTextureLoader().load(
      sides.map((x) => LDR.Colors.envMapPrefix + x + ".jpg"),
      updateEnvMaps,
    );
  }
};
  ...
  if (color.material) {
    ...
    if (color.material === "CHROME" || color.material === "METAL") {
      ...
      registerTextureListener = (m) =>
        LDR.Colors.listeningMaterials.metal.push(m);
      params.envMapIntensity = 1.0;
    } else if (color.material === "RUBBER") {
      params.roughness = 0.9;
      registerTextureListener = (m) =>
        LDR.Colors.listeningMaterials.rubber.push(m);
    } else if (color.material === "PEARLESCENT") {
      registerTextureListener = (m) =>
        LDR.Colors.listeningMaterials.pearl.push(m);
      ...
      if (m.startsWith("SPECKLE VALUE ")) {
        ...
        if (m.length === 5) {
          ...
          registerTextureListener = (m) =>
            LDR.Colors.listeningMaterials.speckle[colorID].push(m);
        } else {
          console.warn(
            "Failed to parse speckle definition for color " +
              colorID +
              ": " +
              m.join("/"),
          );
        ...
        if (m.length === 5) {
          ...
          console.warn(
            "Failed to parse glitter definition for color " +
              colorID +
              ": " +
              m.join("/"),
          );
      ...
      console.warn(
        "Unknown material composition for color " +
          colorID +
          " -> " +
          color.material,
      );
    ...
    registerTextureListener = (m) =>
      LDR.Colors.listeningMaterials.opaque.push(m);
    params.specular = 0xdfdfdf; // Increase glare
    params.shininess = 82;
    params.reflectivity = 0.9;
  } else if (color.alpha > 0) {
    registerTextureListener = (m) =>
      LDR.Colors.listeningMaterials.trans.push(m);
