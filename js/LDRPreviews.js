...
LDR.Previews.prototype.draw = function (id) {
  ...
  if (!pt.baseObject) {
    ...
    let mc = (pt.pliMC = new LDR.MeshCollector(
      opaqueObject,
      sixteenObject,
      transObject,
    ));
    ...
    let [dx, dy] = this.measurer.measure(
      mc.boundingBox,
      baseObject.matrixWorld,
    );
  ...
  this.camera.zoom =
    (this.w * window.devicePixelRatio) / pt.baseObject.subjectSize;
...
LDR.Previews.prototype.createBigPreview = function () {
  ...
  this.ldrButtons = new LDR.Buttons(actions, canvas.parentNode);
