...
LDR.Options.saveOptionsToCookie = function () {
  ...
  function addToKv(v) {
    document.cookie =
      ...
      "=" +
      options[v] +
      "; SameSite=Strict; expires=Wed, 3 Jun 2122 12:00:01 UTC; path=/";
  }
...
LDR.Options.appendHeader = function (optionsBlock) {
  ...
  window.onscroll = function () {
    ...
    if (
      document.body.scrollTop > boundary ||
...
LDR.Options.appendDescriptionBar = function (
  optionsBlock,
  columns,
  description,
) {
...
LDR.Options.appendPartColorOptions = function (optionsBlock) {
  let group = this.addOptionsGroup(
    optionsBlock,
    2,
    "Background and Point Color",
  );
  let options = this;
  ...
  function createPreview(parent, forBG) {
    ...
    if (forBG) options.createSvgBlock(0, 0, true, oldColor, lineColor, svg);
    else options.createSvgPoints(0, 0, pointColor, svg, 2);
...
LDR.Options.createButtons = function (
  parent,
  numberOfButtons,
  initiallySelected,
  onChange,
  ...
  for (let i = 0; i < numberOfButtons; i++) {
    ...
    button.setAttribute(
      "class",
      i === initiallySelected ? "option_selected" : "option",
    );
...
LDR.Options.createSvgBlock = function (
  x,
  y,
  closed,
  getFillColor,
...
LDR.Options.createSvgCylinder = function (
  x,
  y,
  highContrast,
  getFillColor,
  ...
  let center = LDR.SVG.makeRect(
    x - dx2,
    y - dy / 2,
    LDR.Options.svgBlockWidth,
    dy,
  );
