...
THREE.LDRLoader = function (onLoad, storage, options) {
      ...
      toBeFetched.forEach((id) => {
        ...
        if (pt) {
          ...
          pt.steps.forEach((step) =>
            step.subModels.forEach((sm) => stillToBeFetched.push(sm.ID)),
          );
          improved = true;
        } else {
  ...
  this.loader = new THREE.FileLoader(
    this.options.manager || THREE.DefaultLoadingManager,
  );
...
THREE.LDRLoader.prototype.parseColor = function (colorID) {
  ...
  if (LDR.Colors[colorID] === undefined) {
    ...
    this.onWarning({
      message:
        'Unknown color "' + colorID + '". Black (0) will be shown instead.',
      line: i,
      subModel: part.ID,
    });
    return 0;
...
THREE.LDRLoader.prototype.parse = function (data, defaultID) {
  ...
  for (let i = 0; i < dataLines.length; i++) {
    ...
    if (!part.modelDescription && modelDescription) {
      ...
      if (modelDescription.startsWith("~Unknown part ")) {
        // TODO: This piece of code is specific to Brickhub.org and should be generalised.
        self.onError({
          message:
            'Unknown part "' +
            part.ID +
            '". Please <a href="../upload.php">upload</a> this part for it to be shown correctly in this model. If you do not have it, perhaps you can find it <a href="https://www.ldraw.org/cgi-bin/ptscan.cgi?q=' +
            part.ID +
            '">here on LDraw.org</a>. For now it will be shown as a cube. <a href="#" onclick="bump();">Click here</a> once the part has been uploaded to load it into the model.',
          line: i,
          subModel: part.ID,
        });
      }
    ...
    switch (lineType) {
      case 0:
        ...
        function handleFileLine(originalFileName) {
          ...
          if (isEmpty && !self.mainModel) {
            ...
            console.warn(
              "Special case: Main model ID change from " +
                part.ID +
                " to " +
                fileName,
            );
            ...
            // Close model and start new as no FILE directive has been encountered:
            closeStep(false);
            ...
            if (!part.ID) {
              ...
              console.warn(
                originalFileName,
                "No ID in main model - setting default ID",
                defaultID,
              );
              console.dir(part);
              console.dir(step);
        ...
        if (is("FILE")) {
          ...
          // Special case where some very old files use '0 file' instead of the proper '0 FILE':
          handleFileLine(parts.slice(2).join(" "));
          saveThisCommentLine = false;
        } else if (is("Name:")) {
          ...
          if (parts.length >= 5) {
            step.rotation = new THREE.LDRStepRotation(
              parts[2],
              parts[3],
              parts[4],
              parts.length === 5 ? "REL" : parts[5],
            );
          ...
          if (texmapPlacement) {
            // Expect "0 !TEXMAP FALLBACK" or "0 !TEXMAP END"
            if (
              !(
                parts.length === 3 &&
                (parts[2] === "FALLBACK" || parts[2] === "END")
              )
            ) {
              self.onWarning({
                message:
                  'Unexpected !TEXMAP line. Expected FALLBACK or END line. Found: "' +
                  line +
                  '".',
                line: i,
                subModel: part.ID,
          ...
          // Inline texmap : https://www.ldraw.org/article/47.html
          if (parts[2] !== "START") {
            // File name on !DATA line (not '0 !DATA START' from preliminary versions):
            handleFileLine(parts.slice(2).join(" "));
          }
          ...
          let detectMimetype = (id) =>
            id.endsWith("jpg") || id.endsWith("jpeg") ? "jpeg" : "png"; // Only png supported according to the spec.
          let pid = part.ID;
          ...
          part.preview = new THREE.LDRPartDescription(
            colorID,
            position,
            rotation,
          );
        ...
        if (saveThisCommentLine) {
          ...
          if (step.subModels.length > 0) {
            step.subModels[step.subModels.length - 1].commentLines.push(
              fileLine,
            );
          } else {
      ...
      case 1: // 1 <colour> x y z a b c d e f g h i <file>
        ...
        let subModelID = parts
          .slice(14)
          .join(" ")
          .toLowerCase()
          .replace("\\", "/");
        let subModel = new THREE.LDRPartDescription(
          colorID,
          position,
          rotation,
          subModelID,
          part.certifiedBFC && localCull,
          invertNext,
          texmapPlacement,
        );

        (inTexmapFallback ? texmapPlacement.fallback : step).addSubModel(
          subModel,
        );
      ...
      case 2: // Line "2 <colour> x1 y1 z1 x2 y2 z2"
        ...
        (inTexmapFallback ? texmapPlacement.fallback : step).addLine(
          colorID,
          p1,
          p2,
          texmapPlacement,
      ...
      case 3: // 3 <colour> x1 y1 z1 x2 y2 z2 x3 y3 z3 [u1 v1 u2 v2 u3 v3]
        ...
        (inTexmapFallback ? texmapPlacement.fallback : step).addTriangle(
          colorID,
          p1,
          p2,
          p3,
      ...
      case 4: // 4 <colour> x1 y1 z1 x2 y2 z2 x3 y3 z3 x4 y4 z4
        ...
        (inTexmapFallback ? texmapPlacement.fallback : step).addQuad(
          colorID,
          p1,
          p2,
          p3,
      ...
      case 5: // Conditional lines:
        ...
        (inTexmapFallback ? texmapPlacement.fallback : step).addConditionalLine(
          colorID,
          p1,
          p2,
          p3,
  ...
  if (this.storage.db) {
    if (
      ...
      self.storage.saveInstructionsToStorage(
        self,
        self.options.key,
  ...
  if (LDR.TexmapPlacements.length > 0) {
    ...
    LDR.TexmapPlacements.forEach((tmp) => {
      ...
      if (!self.texmaps.hasOwnProperty(file)) {
        ...
        self.texmapLoader.load(
          self.idToTextureUrl(file),
          (t) => setTexture(t, file),
          undefined,
...
THREE.LDRLoader.prototype.onPartsLoaded = function (loadedParts) {
  ...
  function checkPart(id) {
    if (
      !(
        self.partTypes.hasOwnProperty(id) || unloadedPartsSet.hasOwnProperty(id)
      )
    ) {
  ...
  loadedParts.forEach((pt) =>
    pt.steps.forEach((s) => s.subModels.forEach((sm) => checkPart(sm.ID))),
  );
  ...
  loadedParts.forEach((pt) => {
    if (pt.steps.length === 0 && pt.ID !== "empty.dat") self.purgePart(pt.ID);
  });
...
THREE.LDRLoader.prototype.toLDR = function () {
  ...
  this.applyOnPartTypes((pt) => {
    if (seen.hasOwnProperty(pt.ID) && !(pt.inlined || pt.isOfficialLDraw())) {
      ret += pt.toLDR(self);
  ...
  this.texmapDataurls.forEach((obj) =>
    outputDataUrl(obj.id, obj.mimetype, obj.content),
  );
...
THREE.LDRLoader.prototype.unpack = function (obj) {
  ...
  for (let i = 0; i < numberOfTexmaps; i++) {
    ...
    if (tmp.idx !== LDR.TexmapPlacements.length) {
      console.error(
        "Indexing error on packed texmap. Expected " +
          LDR.TexmapPlacements.length +
          ", found " +
          tmp.idx,
      );
    }
  ...
  names.forEach((name, i) => {
    ...
    for (let j = 0; j < numSteps; j++) {
      let step = new THREE.LDRStep();
      [idxI, idxF, idxS] = step.unpackFrom(
        arrayI,
        arrayF,
        arrayS,
        idxI,
...
THREE.LDRLoader.prototype.pack = function () {
  ...
  LDR.TexmapPlacements.forEach((tmp) =>
    tmp.packInto(arrayI, arrayF, arrayS, nameMap),
  );
...
*/
THREE.LDRPartDescription = function (
  colorID,
  position,
  rotation,
  ID = "",
...
THREE.LDRPartDescription.prototype.cloneColored = function (colorID) {
  ...
  let ret = new THREE.LDRPartDescription(
    c,
    this.p,
    this.r,
    this.ID,
...
THREE.LDRStepRotation.prototype.getRotationMatrix = function (defaultMatrix) {
  ...
  rotationMatrix.set(a, b, c, 0, d, e, f, 0, g, h, i, 0, 0, 0, 0, 1);
...
THREE.LDRStep.prototype.hasPrimitives = function () {
  return (
    this.lines.length > 0 ||
    this.conditionalLines.length > 0 ||
...
THREE.LDRStep.prototype.packInto = function (
  arrayI,
  arrayF,
  arrayS,
  subModelMap,
  ...
  function handleSubModel(sm) {
    ...
    arrayI.push(
      subModelMap[sm.ID] * 4 + (sm.invertCCW ? 2 : 0) + (sm.cull ? 1 : 0),
    ); // Encode these three properties into a single int.
    if (saveCommentLines) {
...
THREE.LDRStep.prototype.unpack = function (obj) {
  ...
  for (let i = 0; i < numberOfTexmapPlacements; i++) {
    ...
    [idxI, idxF, idxS] = texmapPlacement.unpackFrom(
      arrayI,
      arrayF,
      arrayS,
      idxI,
...
THREE.LDRStep.prototype.unpackFrom = function (
  arrayI,
  arrayF,
  arrayS,
  idxI,
...
THREE.LDRStep.prototype.cloneColored = function (colorID) {
  ...
  ret.subModels = this.subModels.map((subModel) =>
    subModel.cloneColored(colorID),
  );
...
THREE.LDRStep.prototype.toLDR = function (
  loader,
  prevStepRotation,
  isLastStep,
) {
  ...
  if (!this.rotation) {
    ...
    // We have a rotation. Check against prev:
...
THREE.LDRStep.prototype.addTriangle = function (
  c,
  p1,
  p2,
  p3,
  ...
  this.triangles.push(
    new LDR.Line3(c, p1, p2, p3, cull, invert, texmapPlacement),
  );
  texmapPlacement && texmapPlacement.use();
};

THREE.LDRStep.prototype.addQuad = function (
  c,
  p1,
  p2,
  p3,
  ...
  this.quads.push(
    new LDR.Line4(c, p1, p2, p3, p4, cull, invert, texmapPlacement),
  );
  texmapPlacement && texmapPlacement.use();
};

THREE.LDRStep.prototype.addConditionalLine = function (
  c,
  p1,
  p2,
  p3,
...
THREE.LDRStep.prototype.generateThreePart = function (
  loader,
  colorID,
  position,
  rotation,
...
THREE.Vector3.prototype.toLDR = function () {
  return (
    LDR.convertFloat(this.x) +
    " " +
    LDR.convertFloat(this.y) +
    " " +
    LDR.convertFloat(this.z)
  );
};

THREE.Matrix3.prototype.toLDR = function () {
  let e = this.elements;
  let rowMajor = [e[0], e[3], e[6], e[1], e[4], e[7], e[2], e[5], e[8]];
...
LDR.Line2.prototype.pack = function (arrayI, arrayF) {
  ...
  arrayF.push(this.p1.x, this.p1.y, this.p1.z, this.p2.x, this.p2.y, this.p2.z);
};

LDR.Line2.prototype.unpackFrom = function (
  arrayI,
  arrayF,
  idxI,
  idxF,
...
LDR.Line3.prototype.unpackFrom = function (
  arrayI,
  arrayF,
  idxI,
  idxF,
...
LDR.Line4.prototype.unpackFrom = function (
  arrayI,
  arrayF,
  idxI,
  idxF,
...
LDR.Line5.prototype.unpackFrom = function (
  arrayI,
  arrayF,
  idxI,
  idxF,
...
THREE.LDRPartType.prototype.setReferencedFrom = function (ldrLoader) {
  ...
  this.steps.forEach((step) =>
    step.subModels.forEach((sm) => handle(sm, step.idx)),
  );
};

THREE.LDRPartType.prototype.canBePacked = function () {
  return (
    (!this.inlined || this.inlined === "OFFICIAL") && // Only pack official parts (not 'GENERATED' (from LDRGenerator) or 'IDB' (unpacked from IndexedDB).
    this.isPart && // Should be a part.
    [
      "Redistributable under CCAL BY 4.0 : see CAreadme.txt",
  ...
  ); // Double-check that it is official.
};
...
THREE.LDRPartType.prototype.purgePart = function (ID) {
  ...
  this.steps.forEach(
    (step) => (step.subModels = step.subModels.filter((sm) => sm.ID !== ID)),
  );
...
THREE.LDRPartType.prototype.removePrimitivesAndSubParts = function (
  loader,
  parentID,
) {
...
THREE.LDRPartType.prototype.addStep = function (step) {
  ...
  if (step.rotation && step.rotation.type === "ADD") {
    if (!this.lastRotation) {
      ...
      step.rotation = new THREE.LDRStepRotation(
        step.rotation.x + this.lastRotation.x,
  ...
  let sameRotation = THREE.LDRStepRotation.equals(
    step.rotation,
    this.lastRotation,
  );
...
THREE.LDRPartType.prototype.generateThreePart = function (
  loader,
  c,
  p,
  r,
  ...
  if (!this.geometry) {
    if (this.isPart) {
      if (taskList) {
        let self = this;
        taskList.push(() =>
          self.generateThreePart(loader, c, p, r, cull, inv, mc, pd),
        );
      ...
      this.steps.forEach((step) =>
        step.generateThreePart(loader, c, p, r, cull, inv, mc, taskList),
      );
      return;
    }
  }
  ...
  for (let tc in this.geometry.lineGeometries) {
    ...
    let normalLines = new THREE.LineSegments(
      this.geometry.lineGeometries[tc],
      material,
    );
  ...
  for (let tc in this.geometry.conditionalLineGeometries) {
    ...
    let conditionalLines = new THREE.LineSegments(
      this.geometry.conditionalLineGeometries[tc],
      material,
    );
...
THREE.LDRPartType.prototype.countParts = function (loader) {
  ...
  this.cnt = this.steps
    .map((step) => step.countParts(loader))
    .reduce((a, b) => a + b, 0);
  return this.cnt;
};
...
LDR.TexmapPlacement = function () {
  // Can be set either from parts when read from LDraw file, or from packed indexeddb object.
...
LDR.TexmapPlacement.prototype.setFromParts = function (parts) {
  ...
  if (parts[3] === "PLANAR") {
    ...
    this.error =
      "Unexpected method in !TEXMAP command or not enough parameters";
    return;
  }
...
LDR.TexmapPlacement.prototype.packInto = function (
  arrayI,
  arrayF,
  arrayS,
  subModelMap,
...
LDR.TexmapPlacement.prototype.unpackFrom = function (
  arrayI,
  arrayF,
  arrayS,
  idxI,
  ...
  for (let i = 0; i < 3; i++) {
    this.p.push(
      new THREE.Vector3(arrayF[idxF++], arrayF[idxF++], arrayF[idxF++]),
    );
  }
  ...
  [idxI, idxF, idxS] = this.fallback.unpackFrom(
    arrayI,
    arrayF,
    arrayS,
    idxI,
...
LDR.TexmapPlacement.prototype.toLDR = function (lines, loader) {
  ...
  let method =
    this.type === 0 ? "PLANAR" : this.type === 1 ? "CYLINDRICAL" : "SPHERICAL";
...
LDR.MeshCollector.prototype.addLines = function (
  color,
  mesh,
  part,
  conditional,
...
LDR.MeshCollector.prototype.attachGlowPasses = function (
  w,
  h,
  scene,
  camera,
      ...
      (obj.mesh.visible =
        ...
        (old ||
          !(
            ...
            obj.part &&
            obj.part.original &&
            obj.part.original.ghost
          ))),
  ); // Do not show faces for ghosted parts.
};
...
LDR.MeshCollector.prototype.colorLinesLDraw = function () {
  this.lineMeshes.forEach((mesh) => {
    ...
    if (c === 272 || c === -273) {
      // Dark Blue override
      m.uniforms.color.value = LDR.Colors.getHighContrastColor4(c);
    } else {
      m.uniforms.color.value = LDR.Colors.getColor4(c);
    }
  });
};
...
LDR.MeshCollector.prototype.overwriteColor = function (color) {
  ...
  function handle(obj) {
    ...
    let isDarkBlue = c === 272 || c === -273;
    let color4 =
      edge && ((LDR.Options && LDR.Options.lineContrast === 0) || isDarkBlue)
        ? LDR.Colors.getHighContrastColor4(c)
        : LDR.Colors.getColor4(c);
...
LDR.MeshCollector.prototype.setVisible = function (v) {
  if (this.visible === v && this.old) {
    // If not old, ghosting might have changed.
    return;
  }
  this.visible = v;
  this.updateMeshVisibility();
...
LDR.MeshCollector.prototype.getGhostedParts = function () {
  let lineObjects = this.lineMeshes.filter(
    (obj) => obj.part && obj.part.original.ghost,
  );
  let triangleObjects = this.triangleMeshes.filter(
    (obj) => obj.part && obj.part.original.ghost,
  );
