 ...
 */
LDR.STORAGE.prototype.retrievePartsFromStorage = function (
  loader,
  parts,
  onDone,
) {
  if (parts.length === 0 || !this.db) {
    // Ensure onDone is called when nothing is fetched or can be fetched.
    onDone([]);
    return;
  }
  let stillToBeBuilt = [];
  ...
  function onHandled(partID) {
    ...
    if (part) {
      // Remember onHandled is also called on error, where part will not be set.
      function checkSubModel(sm) {
        if (
          !(
            loader.partTypes.hasOwnProperty(sm.ID) || seen.hasOwnProperty(sm.ID)
          )
        ) {
          seen[sm.ID] = true;
          remaining++;
    ...
    if (remaining === 0) {
      if (stillToBeBuilt.length > 0) {
        console.warn(
          stillToBeBuilt.length +
            " part(s) could not be fetched from indexedDB: " +
            stillToBeBuilt.slice(0, 10).join("/") +
            "...",
        );
      }
...
LDR.STORAGE.prototype.retrieveInstructionsFromStorage = function (
  loader,
  onDone,
) {
  if (
    !loader.options.hasOwnProperty("key") ||
    !loader.options.hasOwnProperty("timestamp") ||
    !this.db
  ) {
    onDone(false);
    return;
  ...
  request.onerror = function (event) {
    console.warn(
      shortPartID + " retrieval error from indexedDB for key " + key,
    );
    console.dir(event);
    onDone(false);
  };
  request.onsuccess = function (event) {
    let result = request.result;
    if (result && result.timestamp === timestamp && false) {
      try {
        let parts = loader.unpack(result);
        onDone(true, parts);
        console.log(
          "Instructions with key " + key + " read and unpacked from indexedDB!",
        );
      } catch (e) {
        loader.onWarning({
          message:
            "Error during unpacking of instructions from indexedDB: " + e,
          subModel: key,
        });
        onDone(false);
      }
      ...
      console.log(
        'IndexedDB did not contain a current version of instructions with key "' +
          key +
          '" - new instructions will be fetched.',
      );
      onDone(false);
    }
  };
...
LDR.STORAGE.prototype.saveInstructionsToStorage = function (
  loader,
  key,
  timestamp,
) {
  ...
  objectStore.put(packed).onsuccess = () =>
    console.log("Instructions saved to indexedDB.");
};
