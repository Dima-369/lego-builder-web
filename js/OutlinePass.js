...
let OutlinePass = function ( resolution, scene, camera, selectedObjects ) {
    ...
    this.hiddenEdgeColor = new THREE.Color( 0, 1, 0 );
    this.visibleEdgeColor2 = new THREE.Color( 1, 1, 0 );
    this.hiddenEdgeColor2 = new THREE.Color(1, 1, 1 );
    this.edgeGlow = 0.0;
    this.usePatternTexture = false;
    // modify this if the edge should be thicker or thinner, values between 1.0 and 5.0 are recommended
    this.edgeThickness = 5.0;
    this.edgeStrength = 20.0;
    this.downSampleRatio = 1.5;
    this.pulsePeriod = 1;
...
OutlinePass.prototype = Object.assign( Object.create( THREE.Pass.prototype ), {
	...
	render( renderer, writeBuffer, readBuffer, deltaTime, maskActive ) {
		if ( this.selectedObjects.length > 0 ) {
			...
			this.tempPulseColor2.copy( this.tempPulseColor1 ); // Force hidden to match visible

			if ( this.pulsePeriod > 0 ) {
				const alpha = ( 1.0 + Math.cos( performance.now() * 0.001 / this.pulsePeriod ) ) / 2.0;
				this.tempPulseColor1.lerp( this.visibleEdgeColor2, alpha );
				this.tempPulseColor2.copy( this.tempPulseColor1 ); // Sync pulse
			}
		...
		fragmentShader:
				...
				void main() {
					...
					gl_FragColor = vec4(visibleEdgeColor, 1.0) * vec4(d);
                                        if(1.0 - visibilityFactor <= 0.001){gl_FragColor.a = 0.0;}
				}`
                } );
