...
LDR.PliPreviewer.prototype.onResize = function () {
  ...
  if (this.showsSomething()) {
    ...
    if (
      !this.pliHighlighted.pliMC.attachGlowPasses(
        w,
        h,
        this.scene,
        this.camera,
...
LDR.PliPreviewer.prototype.showPliPreview = function (icon) {
  if (icon) {
    ...
    if (color.bricklink_name) {
      blA.setAttribute(
        "href",
        "https://www.bricklink.com/catalogItemIn.asp?P=" +
          partIdBricklink +
          "&c=" +
          color.bricklink_id +
          "&in=A",
      ...
      blA.setAttribute(
        "href",
        "https://www.bricklink.com/catalogItem.asp?P=" + partIdBricklink,
      );
    }
    ...
    if (icon.inlined && !isNaN(icon.inlined)) {
      bhA.setAttribute(
        ...
        "../p/part.php?user_id=" +
          icon.inlined +
          "&id=" +
          encodeURI(partIdNoDat),
      );
      blA.style.visibility = blVisibility;
    } else if (
      !(icon.ldraw_org && !icon.ldraw_org.startsWith("Unofficial_")) &&
      (icon.inlined === undefined || icon.inlined === "undefined")
    ) {
      bhA.setAttribute(
        "href",
        "../p/part.php?from=" + this.modelID + "&id=" + encodeURI(partIdNoDat),
      );
      blA.style.visibility = blVisibility;
    } else {
    ...
    document.getElementById("preview_info_color_ldraw").innerHTML =
      color.name + " (" + c + ")";
    document.getElementById("preview_info_color_lego").innerHTML =
      color.lego_name
        ? color.lego_name + " (" + color.lego_id + ")"
        : "Unknown official LEGO color";
    document.getElementById("preview_info_id_bricklink").innerHTML =
      partIdBricklink;
    document.getElementById("preview_info_color_bricklink").innerHTML =
      color.bricklink_name
        ? color.bricklink_name + " (" + color.bricklink_id + ")"
        : "Unknown Bricklink color";
  }
