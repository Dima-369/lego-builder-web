...
ENV.Scene.prototype.setUpGui = function (
  ...
  let gui = new dat.GUI({ autoPlace: false });
  {
    ...
    c.add(this.camera.position, "y", -5 * size.h, 5 * size.h)
      .listen()
      .onChange(r);
  ...
  }
  {
    ...
    if (this.floor) {
      ...
      c.addColor(options.scene.floor, "color")
        .name("floor color")
  ...
  }
  {
    ...
    c2.add(this.baseObject.rotation, "x", -Math.PI, Math.PI)
      .listen()
      .onChange(r);
    c2.add(this.baseObject.rotation, "y", -Math.PI, Math.PI)
      .listen()
      .onChange(r);
    c2.add(this.baseObject.rotation, "z", -Math.PI, Math.PI)
      .listen()
      .onChange(r);
    if (LDR.Colors[16].m) {
      // Be able to set color:
      let choices = {};
      for (let idx in LDR.Colors) {
        if (idx !== "16" && idx !== "24" && LDR.Colors.hasOwnProperty(idx)) {
          ...
          if (
            ...
            // No special materials - only ABS and normal trans:
      ...
      if (canSetColor) {
        c.add(options.model, "color", choices).onChange((idx) =>
          self.setModelColor(idx),
        );
  ...
  }
  {
    ...
    c.addColor(options.hemisphereLight, "groundColor")
      .name("ground color")
...
ENV.Scene.prototype.setSize = function (b) {
  ...
  let w = bump(b.max.x - b.min.x),
    l = bump(b.max.z - b.min.z),
    h = bump(b.max.y - b.min.y);
...
ENV.Scene.prototype.build = function () {
  let b =
    this.mc.boundingBox ||
    new THREE.Box3(new THREE.Vector3(), new THREE.Vector3(1, 1, 1)); // To build scene around.
  this.setSize(b);
...
ENV.Scene.prototype.buildOMRScene = ENV.Scene.prototype.buildStandardScene =
  ENV.Scene.prototype.build; // Backward compatibility
...
ENV.createFloorTexture = function (size, repeats) {
  ...
  for (let y = 0; y < size; y++) {
    ...
    for (let x = 0; x < size; x++) {
      ...
      let DD = 1 - D;
      if (DD < EDGE) {
        // Blend back to blue: dx/dy -> 0 as EDGE -> DD -> 0
        dx *= 1 - (EDGE - DD) / EDGE;
        dy *= 1 - (EDGE - DD) / EDGE;
      }
      ...
      ctx.fillStyle =
        "rgb(" + toColor(dx) + "," + toColor(dy) + "," + toColor(dz) + ")";
